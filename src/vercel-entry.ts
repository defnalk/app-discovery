...
export default async function handler(req: IncomingMessage & { method?: string }, res: ServerResponse) {
  ...
  if (url.pathname === '/leads/clay' || url.pathname === '/leads/targets' || url.pathname === '/leads/export') {
    ...
    // Valid token but no handler for this method on a token-gated path (e.g. a
    // browser GET of the POST-only /leads/clay). Return a clean 405 — these API
    // endpoints are gated ONLY by the token and must never fall through to the
    // dashboard Basic Auth below, which would (confusingly) prompt a web login.
