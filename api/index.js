...
function emit(level, msg, extra) {
  const entry = { ts: (/* @__PURE__ */ new Date()).toISOString(), level, msg, ...extra };
  if (fileLogging) {
    try {
...
var LOG_DIR, fileLogging, LOG_FILE, log;
var init_log = __esm({
  "src/lib/log.ts"() {
    "use strict";
    LOG_DIR = process.env.VERCEL ? "/tmp/logs" : path.join(process.cwd(), "logs");
    fileLogging = true;
    try {
    ...
    LOG_FILE = path.join(LOG_DIR, `${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10)}.jsonl`);
    log = {
      info: (msg, extra) => emit("info", msg, extra),
      warn: (msg, extra) => emit("warn", msg, extra),
      error: (msg, extra) => emit("error", msg, extra),
      /** Every external call goes through this: url, status, duration, attempt. */
      external: (service, url, extra) => emit("external", `${service} ${url}`, { service, url, ...extra })
    };
  }
});
      ...
      then(onfulfilled, onrejected) {
        ...
        const executeWithRetry = async () => {
          ...
          while (true) {
            ...
            if (attemptCount > 0) headers2["X-Retry-Count"] = String(attemptCount);
            let res$1;
            try {
  ...
  "node_modules/@supabase/supabase-js/dist/index.mjs"() {
    ...
    fetchWithAuth = (supabaseKey, supabaseUrl, getAccessToken, customFetch, tracePropagationOptions) => {
      ...
      return async (input, init) => {
        ...
        return fetch$1(input, _objectSpread23(_objectSpread23({}, init), {}, { headers: headers2 }));
      };
    };
...
var FUNNEL_STAGES, SupabaseLeadsDb, LocalLeadsDb, _db;
var init_db = __esm({
  "src/leads/db.ts"() {
    "use strict";
    init_dist4();
    init_log();
    FUNNEL_STAGES = ["raw", "icp_qualified", "pushed", "sendable", "sent", "replied", "meeting"];
    SupabaseLeadsDb = class {
      backend = "supabase";
      sb;
      constructor(url, key) {
      ...
      async insertClassifications(rows) {
      ...
      async listLeadsJoined() {
        const leads = await this.paged(
          (from, to) => this.sb.from("leads").select("*, classifications(jaka_score, market_status, fit_verdict, reason, classified_at)").order("id").range(from, to)
        );
        return leads.map((l) => {
          const cls = (l.classifications ?? []).sort((a, b) => String(b.classified_at).localeCompare(String(a.classified_at)))[0] ?? {};
          const { classifications: _, ...lead } = l;
          return { ...lead, jaka_score: cls.jaka_score ?? null, market_status: cls.market_status ?? null, fit_verdict: cls.fit_verdict ?? null, reason: cls.reason ?? null };
        });
      }
      async updateLeadStages(stages) {
      ...
      async getFunnelRollups() {
        return this.must(this.sb.from("funnel_rollups").select("*"));
      }
      ...
      async listApprovals() {
        return this.must(this.sb.from("approvals").select("batch_id, status, approved_by, note, created_at"));
      }
      ...
      async listActiveConfig() {
        return this.must(this.sb.from("config").select("*").eq("active", true));
      }
      async insertConfigVersion(key, value, updatedBy) {
        const cur = await this.must(this.sb.from("config").select("*").eq("key", key).eq("active", true));
        const version3 = (cur[0]?.version ?? 0) + 1;
        if (cur[0]) await this.must(this.sb.from("config").update({ active: false }).eq("key", key).eq("active", true));
        await this.must(this.sb.from("config").insert({ key, value, version: version3, active: true, updated_at: (/* @__PURE__ */ new Date()).toISOString(), updated_by: updatedBy }));
      }
      ...
      async resolveSuggestion(id, status, by) {
        const rows = await this.must(
          this.sb.from("threshold_suggestions").update({ status, decided_by: by, decided_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", id).eq("status", "pending").select("*")
        );
        return rows[0] ?? null;
      }
      async insertAudit(a) {
        await this.must(this.sb.from("settings_audit").insert(a));
      }
      async listAudit() {
        return this.must(this.sb.from("settings_audit").select("*").order("created_at", { ascending: false }).limit(200));
      }
      async insertStrategySnapshot(data) {
        await this.must(this.sb.from("strategy_snapshots").insert({ data, computed_at: (/* @__PURE__ */ new Date()).toISOString() }));
      }
    ...
    LocalLeadsDb = class {
      backend = "local";
      file = path2.join(process.cwd(), "data", "leads-local.json");
      d;
      constructor() {
        mkdirSync2(path2.dirname(this.file), { recursive: true });
        this.d = existsSync(this.file) ? JSON.parse(readFileSync(this.file, "utf8")) : { leads: [], classifications: [], campaigns: [], campaign_leads: [], events: [], config: [], threshold_suggestions: [], approvals: [], settings_audit: [], funnel_rollups: [], runs: [], _suggestionSeq: 1 };
      }
      save() {
      ...
      async listLeadsJoined() {
        ...
        return this.d.leads.map((l) => {
          ...
          return { ...l, jaka_score: c?.jaka_score ?? null, market_status: c?.market_status ?? null, fit_verdict: c?.fit_verdict ?? null, reason: c?.reason ?? null };
        });
      }
      async updateLeadStages(stages) {
        for (const l of this.d.leads) {
          const s = stages.get(l.id);
          if (s) l.stage = s;
        }
        this.save();
      }
      ...
      async getFunnelRollups() {
        return this.d.funnel_rollups;
      }
      async listCampaigns(status) {
        return this.d.campaigns.filter((c) => !status || c.status === status);
      }
      async listCampaignLeads(campaignId) {
        return this.d.campaign_leads.filter((c) => !campaignId || c.campaign_id === campaignId);
      }
      async setCampaignStatus(id, status, approvedAt) {
        const c = this.d.campaigns.find((c2) => c2.id === id);
        if (c) {
          c.status = status;
          if (approvedAt) c.approved_at = approvedAt;
        }
        this.save();
      }
      async setCampaignLeadPushed(campaignId, leadId, instantlyLeadId, sendStatus) {
        ...
        if (cl) {
          cl.instantly_lead_id = instantlyLeadId;
          cl.send_status = sendStatus;
        }
        this.save();
      }
      async insertApproval(a) {
        this.d.approvals.push({ ...a, created_at: (/* @__PURE__ */ new Date()).toISOString() });
        this.save();
      }
      async listApprovals() {
        return this.d.approvals;
      }
      async upsertEvents(rows) {
        const key = (e) => `${e.lead_id}|${e.campaign_id}|${e.type}|${e.occurred_at}`;
        const seen = new Set(this.d.events.map(key));
        let n = 0;
        for (const r of rows) {
          if (!seen.has(key(r))) {
            seen.add(key(r));
            this.d.events.push(r);
            n++;
          }
        }
        this.save();
      ...
      async listEvents(sinceIso) {
        return this.d.events.filter((e) => !sinceIso || e.occurred_at >= sinceIso);
      }
      async listActiveConfig() {
        return this.d.config.filter((c) => c.active);
      }
      async insertConfigVersion(key, value, updatedBy) {
        const cur = this.d.config.find((c) => c.key === key && c.active);
        if (cur) cur.active = false;
        this.d.config.push({ key, value, version: (cur?.version ?? 0) + 1, active: true, updated_at: (/* @__PURE__ */ new Date()).toISOString(), updated_by: updatedBy });
        this.save();
      }
      async listSuggestions(status) {
        return this.d.threshold_suggestions.filter((s) => !status || s.status === status);
      }
      async insertSuggestion(s) {
        if (this.d.threshold_suggestions.some((p) => p.status === "pending" && JSON.stringify(p.proposed) === JSON.stringify(s.proposed))) return;
        this.d.threshold_suggestions.push({ ...s, id: this.d._suggestionSeq++, status: "pending", decided_by: null, decided_at: null, created_at: (/* @__PURE__ */ new Date()).toISOString() });
        this.save();
      }
      async resolveSuggestion(id, status, by) {
        const s = this.d.threshold_suggestions.find((s2) => s2.id === id && s2.status === "pending");
        if (!s) return null;
        s.status = status;
        s.decided_by = by;
        s.decided_at = (/* @__PURE__ */ new Date()).toISOString();
        this.save();
        return s;
      }
      async insertAudit(a) {
        this.d.settings_audit.unshift({ ...a, created_at: (/* @__PURE__ */ new Date()).toISOString() });
        this.save();
      }
      async listAudit() {
        return this.d.settings_audit;
      }
      async insertStrategySnapshot(data) {
        this.d.strategy_snapshots ??= [];
        this.d.strategy_snapshots.push({ computed_at: (/* @__PURE__ */ new Date()).toISOString(), data });
        this.save();
      }
      async getLatestStrategySnapshot() {
        const snaps = this.d.strategy_snapshots ?? [];
      ...
      async recordRun(stage, startedAt, counts, notes) {
        this.d.runs.push({ stage, started_at: startedAt, finished_at: (/* @__PURE__ */ new Date()).toISOString(), ...counts, notes });
        this.save();
      }
    };
    _db = null;
...
// src/lib/http.ts
async function throttle(service, minGapMs) {
...
var init_http = __esm({
  "src/lib/http.ts"() {
    ...
    lastCall = /* @__PURE__ */ new Map();
  }
});

// src/leads/instantly.ts
function headers() {
...
var BASE, instantlyEnabled;
var init_instantly = __esm({
  "src/leads/instantly.ts"() {
    "use strict";
    init_http();
    BASE = "https://api.instantly.ai/api/v2";
    instantlyEnabled = () => Boolean(process.env.INSTANTLY_API_KEY);
  }
});
...
var embedJson = (v) => JSON.stringify(v).replace(/</g, "\\u003c");
...
var STAGE_LABELS = {
  ...
  pushed: "Pushed to Instantly",
  sendable: "Clean sendable",
  sent: "Sent",
  replied: "Replied",
  meeting: "Meeting booked"
};
var fmtDate = (s) => s ? s.slice(0, 10) : "\u2013";
var pct = (n, d) => d ? (n / d * 100).toFixed(1) + "%" : "\u2013";
...
function callerBrief(r){
  ...
  const links=[];
  if(r.domain) links.push('<a href="https://'+escq(r.domain)+'" target="_blank" style="color:var(--acc)">\u{1F310} '+escq(r.domain)+'</a>');
  if(li) links.push('<a href="'+escq(li)+'" target="_blank" style="color:var(--acc)">\u{1F4BC} LinkedIn</a>');
  ...
  return '<div style="background:#0d1722;border:1px solid var(--acc);border-radius:8px;padding:12px 16px;margin-bottom:14px">'+
    '<div style="display:flex;justify-content:space-between;align-items:baseline;flex-wrap:wrap;gap:8px">'+
      '<h4 style="margin:0;font-size:15px">\u{1F4CB} Caller brief \u2014 '+escq(r.company||'\u2013')+'</h4>'+
      '<span class="dim">'+escq(r.category||'app')+' \xB7 '+escq(r.geo||'\u2013')+(r.market_status?' \xB7 '+escq(r.market_status):'')+'</span></div>'+
    (links.length?'<p style="margin:6px 0">'+links.join(' &nbsp;\xB7&nbsp; ')+'</p>':'')+
    '<p style="margin:6px 0"><b>Contact:</b> '+escq(r.contact_name||'\u2014')+(r.contact_title?' \xB7 '+escq(r.contact_title):'')+
      (r.phone?' \xB7 \u260E <a href="tel:'+escq(r.phone)+'" style="color:var(--acc)">'+escq(r.phone)+'</a>'+(r.phone_src==='legacy'?' <span class="dim">(unverified #)</span>':''):' \xB7 <span class="dim">no phone</span>')+
      (r.email?' \xB7 \u2709 '+escq(r.email):'')+'</p>'+
...
async function approvalsPage(_req, res, url) {
  const db = getLeadsDb();
  const [pending, leads, tiers, approvals] = await Promise.all([
    db.listCampaigns("pending_approval"),
    db.listLeadsJoined(),
    getTiers(),
    db.listApprovals()
  ]);
  ...
  const batchHtml = await Promise.all(pending.map(async (c) => {
    ...
    const batchLeads = cls.map((cl) => leadById.get(cl.lead_id)).filter((l) => Boolean(l));
    const mix = (key) => {
      const m = /* @__PURE__ */ new Map();
      batchLeads.forEach((l) => m.set(key(l), (m.get(key(l)) ?? 0) + 1));
      return [...m.entries()].sort((a, b) => b[1] - a[1]).map(([k, n]) => `${esc(k)}: ${n}`).join(" \xB7 ");
    };
    const scores = batchLeads.map((l) => l.jaka_score).filter((s) => s != null);
    const dist = scores.length ? `min ${Math.min(...scores)} \xB7 median ${scores.sort((a, b) => a - b)[Math.floor(scores.length / 2)]} \xB7 max ${Math.max(...scores)}` : "no scores";
    return `
<div class="panel">
  ...
  const history = approvals.slice(0, 20).map((a) => `<tr><td>${fmtDate(a.created_at)}</td><td>${esc(a.batch_id.slice(0, 8))}</td><td>${a.status === "approved" ? "\u2705 approved" : "\u274C rejected"}</td><td>${esc(a.approved_by)}</td><td>${esc(a.note ?? "")}</td></tr>`).join("");
  const body = `
${msg ? `<div class="panel" style="border-color:var(--good)">${esc(msg)}</div>` : ""}
...
<table><thead><tr><th>Date</th><th>Batch</th><th>Decision</th><th>By</th><th>Note</th></tr></thead><tbody>${history || '<tr><td colspan="5" class="dim">none yet</td></tr>'}</tbody></table></div>`;
  send(res, 200, pageShell({ title: "Leads \xB7 Approval Queue", active: "approvals", body }));
}
async function approveAction(req, res) {
  ...
  for (const cl of toPush) {
    ...
    try {
      const r = await pushLead(campaign.instantly_campaign_id, {
        email: lead.email,
        company_name: lead.company ?? void 0,
  ...
  await db.insertApproval({
    batch_id: batchId,
    lead_ids: toPush.map((c) => c.lead_id),
    excluded_lead_ids: excluded,
    status: "approved",
    approved_by: approvedBy,
    note
  });
  await db.setCampaignStatus(batchId, "approved", (/* @__PURE__ */ new Date()).toISOString());
...
async function performancePage(_req, res) {
  const db = getLeadsDb();
  const [leads, events, tiers] = await Promise.all([db.listLeadsJoined(), db.listEvents(), getTiers()]);
  const leadById = new Map(leads.map((l) => [l.id, l]));
  const armStats = /* @__PURE__ */ new Map();
  const daily = /* @__PURE__ */ new Map();
  for (const e of events) {
    const arm = e.lead_id ? leadById.get(e.lead_id)?.source_arm : void 0;
    if (!arm) continue;
    ...
    if (e.type === "reply" || e.type === "positive_reply") {
      ...
      (daily.get(arm) ?? daily.set(arm, /* @__PURE__ */ new Map()).get(arm)).set(day, (daily.get(arm).get(day) ?? 0) + 1);
    }
  }
  ...
  const replies = events.filter((e) => e.type === "reply" || e.type === "positive_reply").sort((a, b) => b.occurred_at.localeCompare(a.occurred_at)).slice(0, 100).map((e) => {
    const l = e.lead_id ? leadById.get(e.lead_id) : void 0;
    if (!l) return "";
...
<tbody>${auditRows || '<tr><td colspan="6" class="dim">no changes yet</td></tr>'}</tbody></table></div>`;
  send(res, 200, pageShell({ title: "Leads \xB7 Settings", active: "settings", body }));
}
async function resolveSuggestionAction(req, res) {
  const db = getLeadsDb();
  const form = await formBody(req);
  ...
  if (proposed.setting && proposed.change !== void 0) {
    const config = await db.listActiveConfig();
    const old = config.find((c) => c.key === proposed.setting)?.value ?? proposed.current ?? null;
    const merged = typeof old === "object" && old !== null && typeof proposed.change === "object" ? { ...old, ...proposed.change } : proposed.change;
...
async function strategyPage(_req, res) {
  const db = getLeadsDb();
  ...
  if (snap) {
    ...
    if (d.lead_book_by_arm) {
      ...
      const armRows = Object.entries(d.lead_book_by_arm).sort((a, b) => {
        const sum = (x) => Object.values(x).reduce((s, c) => s + c.total, 0);
        return sum(b[1]) - sum(a[1]);
      }).map(([arm, cells]) => {
        const sumUp = (cs) => Object.values(cs ?? {}).reduce((s, c) => ({
          total: s.total + c.total,
          sendable: s.sendable + c.sendable,
          qualified: s.qualified + c.qualified,
          withEmail: s.withEmail + c.withEmail
        }), { total: 0, sendable: 0, qualified: 0, withEmail: 0 });
        const totals = sumUp(cells);
        const prevTotals = pd?.lead_book_by_arm ? sumUp(pd.lead_book_by_arm[arm]) : void 0;
      ...
      armTab = `<div class="panel" style="overflow-x:auto">
        <h3 style="margin-top:0">Leads by sourcing strategy \xD7 geo <span class="dim">(snapshot ${fmtDate(snap.computed_at)}${prev ? ` \xB7 \u0394 vs ${fmtDate(prev.computed_at)}` : " \xB7 deltas appear after the next nightly snapshot"})</span></h3>
        <table><thead><tr><th>Strategy arm</th><th class="num">Total</th>${geos.map((g) => `<th>${g}</th>`).join("")}</tr></thead>
        <tbody>${armRows}</tbody></table>
        <p class="muted-note">s = clean sendable \xB7 q = ICP qualified \xB7 \u25B2\u25BC = change since the previous day's snapshot. The same split drives the A/B readout on Performance once sends begin.</p></div>`;
    }
    const buckets = [.../* @__PURE__ */ new Set([...Object.keys(d.market), ...Object.keys(d.lead_book)])].sort();
    const heat = (hot) => hot >= 100 ? "rgba(255,107,107,.18)" : hot >= 30 ? "rgba(255,179,71,.14)" : "transparent";
    const pd2 = prev?.data ?? null;
    const rows = buckets.map((b) => {
      const cells = [...POOLS, "unknown"].map((g) => {
        ...
        return `<td style="background:${heat(mk?.hot ?? 0)}">
          <b>${lb?.total ?? 0}</b>${delta(lb?.total ?? 0, plb ? plb.total : void 0)} <span class="dim">leads</span> <span class="dim">(${lb?.sendable ?? 0}s/${lb?.qualified ?? 0}q)</span><br>
          <span class="dim">${mk ? `${mk.hot} hot${delta(mk.hot, pmk ? pmk.hot : void 0)} \xB7 ${gap} expanding in` : "no chart data"}</span></td>`;
      }).join("");
...
function registerRoutes(routes2) {
  ...
  routes2.set("GET /leads/approvals", approvalsPage);
